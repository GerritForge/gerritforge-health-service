...
abstract class GitActionTest {
	...
	protected Ref pushNewCommitToBranch() throws GitAPIException {
		Git git = Git.cloneRepository().setURI(gitRepoUri).setDirectory(testCloneRepoPath.toFile()).call();
		RevCommit commit = git.commit().setAuthor(TEST_AUTHOR, TEST_AUTHOR_EMAIL).setMessage(TEST_COMMIT_MESSAGE).call();
