...
class Scraper:
    ...
    def __init__(
        ...
        self.mertics_keys = Scraper.__system_metrics.union(
            self.__metrics_with_repository(Scraper.__git_repo_metrics)
        ).union(self.__metrics_with_repository(Scraper.__git_per_repo_metrics))
    ...
    def scrape_to_dict(
        ...
        if data:
            ...
            existing_samples = self.__only_existing_metrics(samples)
            return (
                existing_samples,
    ...
    def __metrics_with_repository(self, metrics: set[str]):
        return [f"{key}{self.repository}" for key in metrics]

    def __only_existing_metrics(self, metrics: set[str, None | float]):
        return {
