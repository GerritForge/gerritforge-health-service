...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        + f"`--outout-csv-file`, '{Mode.snapshot.value}' where it will ask for metrics and return them as JSON",
    )
    args = parser.parse_args()

    if args.mode == Mode.batch and args.output_csv_file is None:
        parser.error(f"--mode {Mode.batch.value} requires `--outout-csv-file` value.")

    scraper = Scraper(
        args.mode,
        args.repository.replace("-", "_"),
        args.url,
        args.output_csv_file if args.output_csv_file is not None else None,
        args.bearer_token,
    )
